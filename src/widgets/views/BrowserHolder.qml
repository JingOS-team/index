...
QtObject {
    ...
    /**
      *
      */
    property bool visible: (browser.status.code === Maui.FMList.LOADING
                            || browser.status.code === Maui.FMList.ERROR
                            || (browser.status.code === Maui.FMList.READY
                                && browser.status.empty === true))
                           && browser.count <= 0
