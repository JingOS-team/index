...
Item {
    ...
    SplitView.preferredHeight: _splitView.orientation
                               === Qt.Vertical ? _splitView.height
                                                 / (_splitView.count) : _splitView.height
    SplitView.minimumHeight: _splitView.orientation === Qt.Vertical ? 200 : 0

    SplitView.preferredWidth: _splitView.orientation
                              === Qt.Horizontal ? _splitView.width
                                                  / (_splitView.count) : _splitView.width
    ...
    SplitView {
        ...
        handle: Rectangle {
            implicitWidth: 6 * appScaleSize
            implicitHeight: 6 * appScaleSize
        ...
        StackView {
            ...
            initialItem: FileBroswerView {
                ...
                gridItemSize: {
                    ...
                    case 0:
                        return Math.floor(48 * 1.5)
                    case 1:
                        return Math.floor(64 * 1.5)
                    case 2:
                        return Math.floor(80 * 1.5)
                    case 3:
                        return Math.floor(124 * 1.5)
                    default:
                        return Math.floor(96 * 1.5)
                    }
                }
                ...
                onSelectionModeChanged: {
                    ...
                    selectionMode = Qt.binding(function () {
                        return root.selectionMode
                    })
                }
                ...
                Rectangle {
                    ...
                    anchors.left: parent.left
                    anchors.right: parent.right
                    height: 2 * appScaleSize
                    opacity: 1
                    color: Kirigami.Theme.highlightColor
                    visible: _splitView.currentIndex === _index
                             && _splitView.count === 2
                }
                        ...
                        if (appSettings.singleClick)
                        {
                            if (appSettings.previewFiles && item.isdir != "true"
                                ...
                                if (item.mime.indexOf("video") != -1) //视频 直接播放
                                {
                                    ...
                                    root.playVideo(item)
                                    leftMenuData.addFileToRecents(
                                                item.path.toString())
                                } else if (Maui.FM.checkFileType(
                                               Maui.FMList.IMAGE,
                                               item.mime))
                                {
                                    root.imageIndex = index
                                    ...
                                    root.root_musicPage.visible = true
                                    if (root.root_musicPage.playPathList.length > 0)
                                        return
                                    var startIndex = 0
                                    var count = -1
                                    for (var i = 0; i
                                         < root.currentBrowser.currentFMList.count; i++) {
                                        var normalModel = root.currentBrowser.currentFMModel.get(
                                                    i)
                                        if (normalModel.mime.indexOf(
                                                    "audio") != -1) {
                                            root.root_musicPage.pushOnePlayPath(
                                                        normalModel.path)
                                            count++
                                    ...
                                    root.root_musicPage.setCurrentIndex(
                                                startIndex)
                                    root.root_musicPage.setPause(false)
                                } else if (Maui.FM.checkFileType(
                                               Maui.FMList.TEXT,
                                               item.mime)) //如果是音频或者txt文件直接内部打开
                                {
                                    var sizeOut = 100 * 1024
                                    if (item.size > sizeOut) {
                                        root.openWith(item)
                                    } else {
                                        _stackView.push(_previewerComponent,
                                                        StackView.Immediate)
        ...
        Loader {
            id: terminalLoader
            SplitView.fillWidth: true
            SplitView.preferredHeight: 200 * appScaleSize
            SplitView.maximumHeight: parent.height * 0.5
            SplitView.minimumHeight: 100 * appScaleSize
    ...
    function syncTerminal(path) {
        if (terminalLoader.item && terminalVisible)
            terminalLoader.item.session.sendText("cd '" + String(path).replace(
                                                     "file://", "") + "'\n")
    }
